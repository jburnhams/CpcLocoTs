...
class CpcLoco {
	...
	static vmMock = {
		...
		initTest1: function () {
			CpcLoco.vmMock.testStepCounter1 = CpcLoco.vmMock.maxSteps;
			CpcLoco.vmMock.line = 0; // or "start";
			CpcLoco.vmMock.gosubStack.length = 0;
			CpcLoco.vmMock.testVariables1.initAllVariables();

			for (let i = "a".charCodeAt(0); i <= "z".charCodeAt(0); i += 1) {
				const varChar = String.fromCharCode(i);

				CpcLoco.vmMock.testVariables1.setVarType(varChar, "R");
			}
		},

		vmGetAllVariables: function () {
			return CpcLoco.vmMock.testVariables1.getAllVariables();
		},
		vmGetAllVarTypes: function () {
			return CpcLoco.vmMock.testVariables1.getAllVarTypes();
		},
		vmLoopCondition: function () {
			CpcLoco.vmMock.testStepCounter1 -= 1;
			return CpcLoco.vmMock.testStepCounter1 > 0;
		},
		...
		addressOf(variable: string): number { // addressOf operator
			return CpcLoco.vmMock.testVariables1.getVariableIndex(variable);
		},
		...
		vmDefineVarTypes(type: VarTypes, _err: string, first: string, last?: string) {
			...
			for (let i = firstNum; i <= lastNum; i += 1) {
				...
				CpcLoco.vmMock.testVariables1.setVarType(varChar, type);
			}
		},
		...
		dim(varName: string): void { // varargs
			...
			CpcLoco.vmMock.testVariables1.dimVariable(varName, dimensions);
		},
		...
		vmStop: function (reason: string) {
			if (reason === "end") {
				CpcLoco.vmMock.testStepCounter1 = 0;
			}
		},

		vmGoto: function (line: string | number) {
			CpcLoco.vmMock.line = line;
		},
...
if (!myGlobalThis.CpcLoco) {
	myGlobalThis.CpcLoco = CpcLoco;
}
...
function testParseExample(example: ExampleEntry) {
	const codeGeneratorJs = CpcLoco.codeGeneratorJs,
		codeGeneratorToken = CpcLoco.codeGeneratorToken,
		basicTokenizer = CpcLoco.basicTokenizer,
	...
	if (example.meta !== "D" && hasLineNumbers(input)) { // skip data files  && example.meta !== "X" && example.meta !== "Z") { // skip data, dsk and zip files
		...
		if (!output.error) {
			...
			try {
				...
				checks += "(line " + CpcLoco.vmMock.line + ")";
	...
	if (CpcLoco.assert) {
		CpcLoco.assert.ok(!output.error, example.key + ": " + checks);
	}
...
function fnExampleLoaded(error?: Error, code?: string) {
	...
	CpcLoco.fnExampleDone1();

	if (code) {
		fnEval(code); // load example (for nodeJs)
	}

	const key = CpcLoco.model.getProperty<string>(ModelPropID.example),
		example = CpcLoco.model.getExample(key),
...
function testLoadExample(exampleEntry: ExampleEntry) {
	const example = exampleEntry.key,
		fileOrUrl = CpcLoco.dataBaseDirOrUrl + "/" + example + ".js";

	if (CpcLoco.assert) {
		CpcLoco.fnExampleDone1 = CpcLoco.assert.async();
	}
...
function testNextExample() {
	const testExamples = CpcLoco.testExamples,
		testIndex = CpcLoco.testIndex;

	if (testIndex < testExamples.length) {
		const key = testExamples[testIndex];

		CpcLoco.testIndex += 1;
		CpcLoco.model.setProperty(ModelPropID.example, key);
		const example = CpcLoco.model.getExample(key);
...
function fnIndexLoaded(error: Error | undefined, code?: string) {
	...
	CpcLoco.testExamples = Object.keys(allExamples);
	if (TestHelper.config.test) { // a single test specified?
		const testName = TestHelper.config.test;

		CpcLoco.testExamples = CpcLoco.testExamples.filter(function (item: string) {
			return item === testName;
		});
	}
	CpcLoco.testIndex = 0;

	CpcLoco.totalExamples += CpcLoco.testExamples.length;
	if (CpcLoco.assert) {
		CpcLoco.assert.expect(CpcLoco.totalExamples);
	}
...
function testLoadIndex(exampeDb: DatabaseEntry) {
	...
	if (!isUrl(dir)) {
		dir = CpcLoco.baseDir + dir;
	}

	if (CpcLoco.assert) {
		CpcLoco.fnIndexDone1 = CpcLoco.assert.async();
	}
	...
	CpcLoco.dataBaseDirOrUrl = dir;

	const fileOrUrl = CpcLoco.dataBaseDirOrUrl + "/0index.js"; // "./examples/0index.js";
...
function testNextIndex() {
	const databaseNames = CpcLoco.databaseNames,
		databaseIndex = CpcLoco.databaseIndex;
	let nextIndex = false;
	...
	if (!nextIndex) {
		Utils.console.log("testParseExamples: Total examples:", CpcLoco.totalExamples, "/ Ignored examples:", CpcLoco.ignoredExamples);
	}
}
...
function testStart() {
	...
	CpcLoco.initVmMock1();
	CpcLoco.totalExamples = 0;
	CpcLoco.ignoredExamples = 0;
	CpcLoco.databaseNames = CpcLoco.initDatabases();
	CpcLoco.databaseIndex = 0;
	testNextIndex();
}
...
if (typeof myGlobalThis.QUnit !== "undefined") {
	...
	QUnit.module("testParseExamples: Tests", function (/* hooks */) {
		QUnit.test("testParseExamples", function (assert: QUnitAssertType3) {
			CpcLoco.assert = assert;
			testStart();
		});
	});
} else {
	CpcLoco.fnIndexDone1 = function () {
		// empty
	};
	CpcLoco.fnExampleDone1 = function () {
		// empty
	};
