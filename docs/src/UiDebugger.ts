...
export class UiDebugger {
	...
	private updateCallStack() {
		...
		stack.forEach((frame, index) => {
			...
			li.textContent = text;
			li.classList.add("debug-stack-frame");
			...
			li.addEventListener("click", () => {
				...
				if (range) {
					...
					textArea.focus();
					// setAreaSelection sets selection, but focus ensures it's active.
					// Browser behavior typically scrolls to selection on focus/selection change.
				}
			});
